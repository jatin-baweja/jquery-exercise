$(function() {
  ...
  var  newRow = function(productName) {
    ...
    for (var i = 0, j = ratings.length; i < j; i++) {
      $containerDiv.append($('<div class="radio-box"><input class="' + ratings[i].toLowerCase().replace(/ /g, '-') + '" type="radio" disabled /></div>'));
    }
  }
  ...
  $rowSelect = $('#product-ratings').delegate('.product', 'click', function() {
    ...
    $this.toggleClass('highlight').parent().siblings().children('.product').removeClass('highlight');
  ...
  $ratingsContainer.delegate('.rating', 'click', function() {
    ...
    //If a product is selected, check its corresponding radio button and uncheck and disable all others for that product
    if ($products.hasClass('highlight')) {
      console.log($products.filter($('.highlight')).siblings().children('.' + $this.attr('id')).attr('disabled', false).prop('checked', true).parent().siblings().children('input[type="radio"]').attr('disabled', true).prop('checked', false));
    }
  });
});